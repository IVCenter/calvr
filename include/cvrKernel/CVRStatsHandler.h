...
class CVRKERNEL_EXPORT CVRStatsHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        enum StatAddType
        {
            VIEWER_STAT = 0, CAMERA_STAT
        };

        void addStatValue(StatAddType addType, std::string label,
                std::string statName, osg::Vec3 color, std::string collectName,
                bool average = false, bool advanced = false);
        void addStatValueWithLine(StatAddType addType, std::string label,
                std::string statName, osg::Vec3 color, std::string collectName,
                float lineMax, bool average = false, bool advanced = false);
        void addStatTimeBar(StatAddType addType, std::string label,
                std::string statDurationName, std::string statStartTimeName,
                std::string statEndTimeName, osg::Vec3 color,
                std::string collectName, bool advanced = false);
        void addStatTimeBarWithLine(StatAddType addType, std::string label,
                std::string statDurationName, std::string statStartTimeName,
                std::string statEndTimeName, osg::Vec3 color,
                std::string collectName, float lineMax, bool advanced = false);
        void addStatLine(StatAddType addType, std::string statName,
                osg::Vec3 color, std::string collectName, float lineMax,
                bool advanced = false);
    ...
    protected:
        ...
        osg::Geometry* createBackgroundRectangle(const osg::Vec3& pos,
        ...
        osg::Geometry* createTick(const osg::Vec3& pos, float height,
                const osg::Vec4& colour, unsigned int numTicks);

        osg::Node* createCameraTimeStats(std::string& font, osg::Vec3& pos,
                float startBlocks, bool acquireGPUStats, float characterSize,
        ...
        void createTimeBar(osg::Stats * viewerStats, osg::Stats * stats,
        ...
        void calculateStartBlocks(float & startBlocks, float leftPos,
