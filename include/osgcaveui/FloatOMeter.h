...
namespace cui
{
  ...
  class CUIEXPORT FloatOMeter : public Widget, public Events, public DigitListener
  {
    public:
      ...
      bool isDigitMarked() { return _isDigitMarked; }
      DigitLabel* getMarkedDigit() { return _markedDigit; }
      int getMarkedPlace() { return _markedDigitPlace; }
      int getDecimalPlaces() { return _decimalPlace; }
      float getHeight() { return _point->getHeight(); }
      void setMaxIntegerPlaces(int max) { _maxIntegerPlaces = max - 1; }
      void setMinValue(double min) { _minValue = min; }
      void setMaxValue(double max) { _maxValue = max; }
