...
CheckBox::CheckBox(Interaction* interaction) : Button(interaction)
{
  _isChecked = false;
  ...
  if (!_imageChecked || !_imageUnchecked) cerr << "Warning: checkbox icon missing" << endl;
  else updateIcon();
}
...
void CheckBox::setImages(std::string checked, std::string unchecked)
{
  ...
  if (!_imageChecked || !_imageUnchecked) cerr << "Warning: custom checkbox icon missing" << endl;
  else updateIcon();
}
