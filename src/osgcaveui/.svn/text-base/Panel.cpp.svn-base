...
Panel::Panel(Interaction *interaction, Appearance appMode, Movability moveMode) : Widget(), Events(), CardListener()
{
...
Panel::Panel(Interaction *interaction, Appearance appMode, Movability moveMode, int widgetsPerRow) : Widget(), Events(), CardListener()
{
  _interaction = interaction;
  _setPos = false;
  _topLeft.set(0,0,0);
...
Panel::Panel(Interaction *interaction, Vec3 topLeft, Appearance appMode, Movability moveMode) : Widget(), Events(), CardListener()
{
  _interaction = interaction;
  _topLeft = topLeft;
  _setPos = true;
...
void Panel::setVertices()
{
  ...
  tl = _topLeft;
  bl = tl;  bl[1] = tl[1] - _height;
  br = bl;  br[0] = bl[0] + _width;
  tr = br;  tr[1] = tl[1];
...
void Panel::setBGColor()
{
  ...
  if (_appearance==WALL_PANEL)  col[3] = 0.0f;                                // set opacity to 0
  else col[3] = 0.7f;
  (*_BGcolor)[0] = col;
}
