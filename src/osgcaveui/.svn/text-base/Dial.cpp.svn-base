...
Dial::Dial(Interaction* interaction, AdvancedInput input) :
  Card(interaction), CalculatorListener(), FloatOMeterListener()
{
  _input = input;
  ...
  this->setFont(osgText::readFontFile("fonts/arial.ttf"));
...
{
  if (_isInteger) return floor(_value);
  else return _value;
}
...
void Dial::setKnobRange(float newRange)
{
  if (newRange < 0.0f)
  {
    ...
    else _knobRange = 10.0f;
  }
  else _knobRange = newRange;
}
