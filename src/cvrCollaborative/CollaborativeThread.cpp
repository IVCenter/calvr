...
void CollaborativeThread::run()
{
    ...
    while(1)
    {
        ...
        if(_updating)
        {
            ...
            if(_numMessages)
            {
                if(!_socket->send(_messageHeaders,
                        sizeof(CollaborativeMessageHeader) * _numMessages,
                        MSG_NOSIGNAL))
                {
                    return;
                }
                for(int i = 0; i < _numMessages; i++)
                {
                    ...
                    {
                        if(!_socket->send(_messageData[i],
                                _messageHeaders[i].size,MSG_NOSIGNAL))
                        {
                            return;
                        }
            ...
            if(_serverUpdate->numMes)
            {
                ...
                {
                    std::cerr << "type: " << _messageHeaderUpdate[i].type
                            << " target: " << _messageHeaderUpdate[i].target
                            << " size: " << _messageHeaderUpdate[i].size
                            << std::endl;
                    if(_messageHeaderUpdate[i].size)
                    {
                        _messageDataUpdate[i] =
                                new char[_messageHeaderUpdate[i].size];
                        if(!_socket->recv(_messageDataUpdate[i],
                                _messageHeaderUpdate[i].size))
                        {
                            return;
                        }
                    }
            ...
            {
                if(_serverUpdate->masterID >= 0
                        && _serverUpdate->masterID != _id)
                {
                    ...
                    int numBodies =
                            (*_clientInitMap)[_clientUpdate[0].numMes].numHeads
                                    + (*_clientInitMap)[_clientUpdate[0].numMes].numHands;
                    _bodiesUpdate = new BodyUpdate[numBodies];
                    if(!_socket->recv(_bodiesUpdate,
            ...
            else if(_serverUpdate->numUsers > 1)
            {
                ...
                for(int i = 0; i < (_serverUpdate->numUsers - 1); i++)
                {
                    ...
                    numBodies +=
                            (*_clientInitMap)[_clientUpdate[i].numMes].numHeads
                                    + (*_clientInitMap)[_clientUpdate[i].numMes].numHands;
...
void CollaborativeThread::startUpdate(struct ClientUpdate & cu, int numBodies,
        struct cvr::BodyUpdate * bodies, int numMessages,
        CollaborativeMessageHeader * messageHeaders, char** messageData)
{
...
void CollaborativeThread::getUpdate(ServerUpdate * & su,
        ClientUpdate * & clientList, BodyUpdate * & bodyList,
        CollaborativeMessageHeader * & messageHeaders, char ** & messageData)
{
    su = _serverUpdate;
...
void CollaborativeThread::processMessage(CollaborativeMessageHeader & cmh,
        char * data)
{
    switch(cmh.type)
    {
