...
bool TrackerAndroid::init(std::string tag)
{
    _debug =
            ComController::instance()->isMaster() ?
                    ConfigManager::getBool("value","Input.TrackingDebug",false,
                            NULL) :
                    false;
...
TrackerBase::TrackedBody * TrackerAndroid::getBody(int index)
{
    if(index != 0)
    {
        return NULL;
...
unsigned int TrackerAndroid::getButtonMask()
{
    return _mouseButtonMask;
}

float TrackerAndroid::getValuator(int index)
{
    if(index == 0)
    {
...
int TrackerAndroid::getNumBodies()
{
    return 1;
}

int TrackerAndroid::getNumValuators()
{
    return 1;
}

int TrackerAndroid::getNumButtons()
{
    return CVR_NUM_MOUSE_BUTTONS;
}

void TrackerAndroid::update(
        std::map<int,std::list<InteractionEvent*> > & eventMap)
{
    ...
    if(!_handListInit)
    {
        if(_debug)
        {
            std::cerr << "TrackerAndroid: hand list init:" << std::endl;
        }
        ...
        {
            std::cerr
                    << "TrackerAndroid: Error: Unable to find own system in list"
                    << std::endl;
        }
        else
        {
            if(_debug)
            {
                ...
                for(int i = 0; i < TrackingManager::instance()->getNumHands();
                        i++)
                {
                    std::cerr << "Hand: " << i << " buttonFilter: "
                            << TrackingManager::instance()->getButtonFilter(i,
                                    mySystem) << std::endl;
                }
            }
            ...
            {
                TrackingManager::instance()->getHandButtonFromSystemButton(
                        mySystem,i,bhand,bbutton);
            ...
            for(int i = 0; i < _handButtonList.size(); i++)
            {
                ...
                int hsystem, hindex;
                TrackingManager::instance()->getHandAddress(
                        _handButtonList[i].first,hsystem,hindex);
                ...
                if(hsystem >= 0
                        && hsystem
                                < TrackingManager::instance()->getNumTrackingSystems()
                        && TrackingManager::instance()->getTrackingSystem(
                                hsystem) && hindex >= 0
                        && hindex
                                < TrackingManager::instance()->getNumBodies(
                                        hsystem))
                {
            ...
            if(_debug)
            {
                for(int i = 0; i < _handButtonList.size(); i++)
                {
                    std::cerr << "Button: " << i << " hand: "
                            << _handButtonList[i].first << " handButton: "
                            << _handButtonList[i].second << " valid: "
                            << _handValidList[i] << std::endl;
                }
            }
        }
        _handListInit = true;
    ...
    while(InteractionManager::instance()->_mouseQueue.size())
    {
        MouseInteractionEvent * mie =
                dynamic_cast<MouseInteractionEvent*>(InteractionManager::instance()->_mouseQueue.front());
        if(!mie)
        {
        ...
        if(mie->getButton() < 0 || mie->getButton() >= _handButtonList.size()
                || !_handValidList[mie->getButton()])
        {
            tempQueue.push(mie);
            continue;
        }
        ...
        if(TrackingManager::instance()->getHandTrackerType(
                _handButtonList[mie->getButton()].first) == TrackerBase::MOUSE)
        {
        ...
        {
            TrackedButtonInteractionEvent * tie =
            ...
            int hsystem, hindex;
            TrackingManager::instance()->getHandAddress(
                    _handButtonList[mie->getButton()].first,hsystem,hindex);
            if(TrackingManager::instance()->getTrackingSystem(hsystem))
            {
                TrackerBase::TrackedBody * body =
                        TrackingManager::instance()->getTrackingSystem(hsystem)->getBody(
                                hindex);
                tie->setTransform(
                        TrackingManager::instance()->getHandTransformFromTrackedBody(
                                _handButtonList[mie->getButton()].first,body));
            }
