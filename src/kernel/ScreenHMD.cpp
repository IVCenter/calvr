...
ScreenHMD::ScreenHMD() : ScreenBase()
{
}
...
void ScreenHMD::computeViewProj()
{
    ...
    {
	eyeLeft = TrackingManager::instance()->getHeadMat(_myInfo->myChannel->head).getTrans();
    }

    osg::Quat invViewerRot = TrackingManager::instance()->getHeadMat(_myInfo->myChannel->head).getRotate();
    invViewerRot = invViewerRot.inverse();

    //translate screen to origin
    osg::Matrix screenTrans;
    screenTrans.makeTranslate(-(_myInfo->xyz * TrackingManager::instance()->getHeadMat(_myInfo->myChannel->head)));
    ...
    eyeLeft = eyeLeft * screenTrans * osg::Matrix::rotate(invViewerRot) * screenRot;
    ...
    //make view
    _viewLeft = screenTrans * osg::Matrix::rotate(invViewerRot) * screenRot * cameraTrans
	* osg::Matrix::lookAt(osg::Vec3(0, 0, 0), osg::Vec3(0, 1, 0),
		osg::Vec3(0, 0, 1));

    if(_stereo)
    {
	eyeRight = eyeRight * screenTrans * osg::Matrix::rotate(invViewerRot) * screenRot;
	...
	//make view
	_viewRight = screenTrans * osg::Matrix::rotate(invViewerRot) * screenRot * cameraTrans
	    * osg::Matrix::lookAt(osg::Vec3(0, 0, 0), osg::Vec3(0, 1, 0),
		    osg::Vec3(0, 0, 1));
    }
}
