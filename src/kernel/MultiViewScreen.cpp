...
bool MultiViewScreen::handleLineScreenIntersection(IntersectionPlane & plane)
	const {
...
void MultiViewScreen::stencilOutView(const CameraOrient &cam,
    ...
    */
	...
	std::sort(lp_list.begin(), lp_list.end(),
	    MultiViewScreen::sort_by_int);
	std::sort(rp_list.begin(), rp_list.end(),
	    MultiViewScreen::sort_by_int);

	std::set_intersection(lp_list.begin(),
	    lp_list.end(), rp_list.begin(), rp_list.end(),
	    back_inserter(final_list));
...
bool MultiViewScreen::handleCameraScreenIntersection(const CameraOrient &
	cam, GLint ref, GLuint mask, GLuint write_mask) const {
    bool top_left_in = IsInsideOfFrustum(cam, _screenInfoXZ->top_left);
    bool bottom_left_in =
	IsInsideOfFrustum(cam, _screenInfoXZ->bottom_left);
    bool top_right_in = IsInsideOfFrustum(cam, _screenInfoXZ->top_right);
    bool bottom_right_in =
	IsInsideOfFrustum(cam, _screenInfoXZ->bottom_right);
...
void MultiViewScreen::debugStencilBuffer(GLint x, GLint y, GLsizei w,
	GLsizei h, char * file_name) const {
