...
void ScreenBase::defaultCameraInit(osg::Camera * cam)
{
    ...
    cam->setViewport(new osg::Viewport(_myInfo->myChannel->left,
                                           _myInfo->myChannel->bottom,
                                           _myInfo->myChannel->width,
                                           _myInfo->myChannel->height));
    GLenum
            buffer =
                    _myInfo->myChannel->myWindow->gc->getTraits()->doubleBuffer ? GL_BACK
                            : GL_FRONT;
...
osg::Matrix & ScreenBase::getCurrentHeadMatrix(int head)
{
	...
	{
	    _currentHeadMatList[i] = TrackingManager::instance()->getHeadMat(i);
	}
	_headMatListInit = true;
    }

    if(!CVRViewer::instance()->getStopHeadTracking())
    {
...
{
    return osg::Vec3d(-_separation * _eyeSepMult / 2.0,0.0,0.0) * getCurrentHeadMatrix(head);
}
...
void ScreenBase::computeDefaultViewProj(osg::Vec3d eyePos, osg::Matrix & view, osg::Matrix & proj)
{
