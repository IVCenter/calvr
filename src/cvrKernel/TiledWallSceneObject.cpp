...
TiledWallSceneObject::TiledWallSceneObject(std::string name, bool navigation,
        bool movable, bool clip, bool contextMenu, bool showBounds) :
        SceneObject(name,false,movable,clip,contextMenu,showBounds)
{
        ...
        if(_movable && tie->getButton() == _moveButton)
        {
                                    ...
                                    || tie->getInteraction() == BUTTON_UP)))
            {
                if(tie->getInteraction() == BUTTON_DOWN)
                {
                    ...
                    lineP1 = lineP1 * tie->getTransform()
                            * getWorldToObjectMatrix();
                    lineP2 = lineP2 * tie->getTransform()
                            * getWorldToObjectMatrix();

                    linePlaneIntersectionRef(lineP1,lineP2,planePoint,
                            planeNormal,_movePoint,w);
                ...
                {
                    osg::Vec3 wallPoint;
                    if(SceneManager::instance()->getPointOnTiledWall(
                            tie->getTransform(),wallPoint))
                    {
                        osg::Vec3 soPoint = _movePoint
                ...
                else
                {
                    ...
                    lineP1 = lineP1 * tie->getTransform()
                            * _parent->getWorldToObjectMatrix();
                    lineP2 = lineP2 * tie->getTransform()
                            * _parent->getWorldToObjectMatrix();

                    if(linePlaneIntersectionRef(lineP1,lineP2,planePoint,
