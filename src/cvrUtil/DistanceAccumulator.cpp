...
void DistanceAccumulator::pushLocalFrustum()
{
    ...
    localFrustum.transformProvidingInverse(
            currMatrix * _projectionMatrices.back());
    _localFrusta.push_back(localFrustum);

    // Compute new bounding box corners
    bbCornerPair corner;
    corner.second = (currMatrix(0,2) <= 0 ? 1 : 0)
            | (currMatrix(1,2) <= 0 ? 2 : 0) | (currMatrix(2,2) <= 0 ? 4 : 0);
...
void DistanceAccumulator::pushDistancePair(double zNear, double zFar)
{
    if(zFar > 0.0) // Make sure some of drawable is visible
    {
        ...
        if(zNear <= 0.0)
        {
            ...
            if(zNear >= 1.0)
                zNear = 1.0; // 1.0 limit chosen arbitrarily!
        }
        ...
        if(zNear < _limits.first)
            _limits.first = zNear;
        if(zFar > _limits.second)
            _limits.second = zFar;
    }
}
