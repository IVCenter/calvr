...
CVRMulticastSocket::CVRMulticastSocket(MCSocketType st, std::string groupAddress, int port)
{
    _type = st;
	...
	if(::setsockopt(_socket, SOL_SOCKET, SO_REUSEADDR, (const char *)&yes,
		    sizeof(int)) == -1)
	{
	    std::cerr << "CVRMulticastSocket: warning, unable to set SO_REUSEADDR." << std::endl;
	}
...
int CVRMulticastSocket::setsockopt(int level, int optname, void * val, socklen_t len)
{
...
bool CVRMulticastSocket::send(void * buf, size_t len, int flags)
{
    if(_type != SEND)
    {
	std::cerr << "CVRMulticastSocket: error, calling send on a RECV type socket." << std::endl;
	return false;
    }

    if(!buf)
    {
        std::cerr << "CVRMulticastSocket: Error sending NULL buffer." << std::endl;
        return false;
    }

    if(_socket < 0)
    {
        std::cerr << "CVRMulticastSocket: Error: Calling send on invalid socket." << std::endl;
        return false;
    }
    ...
    while(bytesToSend > 0)
    {
        if((sent = sendto(_socket, (const char *)data, bytesToSend, flags,(struct sockaddr *)&_address, sizeof(_address)))
                <= 0)
	{
...
bool CVRMulticastSocket::recv(void * buf, size_t len, int flags)
{
    if(_type != RECV)
    {
	std::cerr << "CVRMulticastSocket: error calling recv from a SEND type socket." << std::endl;
    }

    if(!buf)
    {
        std::cerr << "CVRMulticastSocket: Error recv with NULL buffer." << std::endl;
        return false;
    }

    if(_socket < 0)
    {
        std::cerr << "CVRMulticastSocket: Error: Calling recv on invalid socket." << std::endl;
        return false;
    }
    ...
    while(bytesToRead > 0)
    {
        if((read = recvfrom(_socket, data, bytesToRead, flags,(struct sockaddr *)&_address, &_addrlen)) <= 0)
	{
