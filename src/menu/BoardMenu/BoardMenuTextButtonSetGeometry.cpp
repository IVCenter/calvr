...
void BoardMenuTextButtonSetGeometry::selectItem(bool on)
{
    ...
    if(!on)
    {
	std::map<std::string,TextButtonGeometry*>::iterator oldButton = _buttonMap.find(_intersectedButton);
	if(oldButton != _buttonMap.end())
	{
	    if(!mb->getValue(_intersectedButton))
	    {
		oldButton->second->textTransform->removeChild(oldButton->second->text);
		oldButton->second->textTransform->removeChild(oldButton->second->textSelected);
...
void BoardMenuTextButtonSetGeometry::createGeometry(MenuItem * item)
{
    ...
    if(!mb)
    {
	std::cerr << "Error creating BoardMenuTextButtonSetGeometry." << std::endl;
    }

    _rowHeight = mb->getRowHeight();
    _buttonWidth = (mb->getWidth() - (mb->getButtonsPerRow() - 1) * _spacing) / ((float)(mb->getButtonsPerRow()));
...
void BoardMenuTextButtonSetGeometry::update(osg::Vec3 & pointerStart, osg::Vec3 & pointerEnd)
{
	...
	for(std::map<std::string,TextButtonGeometry*>::iterator it = _buttonMap.begin(); it != _buttonMap.end(); it++)
	{
	    if(isecvec[i].geode == it->second->quad || isecvec[i].geode == it->second->quadSelected)
	    {
		std::map<std::string,TextButtonGeometry*>::iterator oldButton = _buttonMap.find(_intersectedButton);
		if(oldButton != _buttonMap.end())
		{
		...
		{
		    it->second->textTransform->removeChild(it->second->textSelected);
		    it->second->textTransform->removeChild(it->second->text);
		    it->second->textTransform->addChild(it->second->textSelected);
		}
...
BoardMenuTextButtonSetGeometry::TextButtonGeometry * BoardMenuTextButtonSetGeometry::createButtonGeometry(std::string text)
{
    ...
    textd = makeText(text, _textSize, osg::Vec3(0,0,0), _textColorSelected, osgText::Text::CENTER_CENTER);
    tbg->textSelected->addDrawable(textd);

    osg::BoundingBox bb = textd->getBound();
    osg::Vec3 scale, trans;
    float scalef;
    if((_buttonWidth * 0.85) / (bb.xMax() - bb.xMin()) > (_rowHeight * 0.85) / (bb.zMax() - bb.zMin()))
    {
    ...
    osg::Geometry * geo = makeQuad(_buttonWidth, _rowHeight, osg::Vec4(0.2,0.2,0.2,1.0), osg::Vec3(0,-2,-_rowHeight));

    tbg->quad->addDrawable(geo);

    geo = makeQuad(_buttonWidth, _rowHeight, osg::Vec4(0.7,0.7,0.7,1.0), osg::Vec3(0,-2,-_rowHeight));
